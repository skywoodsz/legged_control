...
class TargetTrajectoriesPublisher final {
 public:
  ...
  using VelToTargetTrajectories = std::function<TargetTrajectories(const vector_t& cmd, 
      const SystemObservation& observation, scalar_t& last_height)>;
  ...
  TargetTrajectoriesPublisher(::ros::NodeHandle& nh, const std::string& topicPrefix, CmdToTargetTrajectories goalToTargetTrajectories,
                              VelToTargetTrajectories cmdVelToTargetTrajectories)
    ...
    auto cmdVelCallback = [this](const geometry_msgs::Twist::ConstPtr& msg) {
      ...
      {
        initFlag_ = true;
        last_height_ = latestObservation_.state[8];
      }

      const auto trajectories = cmdVelToTargetTrajectories_(cmdVel, latestObservation_, last_height_);
      targetTrajectoriesPublisher_->publishTargetTrajectories(trajectories);
    };
 ...
 private:
  CmdToTargetTrajectories goalToTargetTrajectories_;
  VelToTargetTrajectories cmdVelToTargetTrajectories_;
