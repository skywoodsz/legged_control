...
KalmanFilterEstimate::KalmanFilterEstimate(PinocchioInterface pinocchioInterface, CentroidalModelInfo info,
  ...
  ros::NodeHandle nh("kf");
  dynamic_srv_ = std::make_shared<dynamic_reconfigure::Server<legged_estimation::ParamConfig>>(nh);
  dynamic_reconfigure::Server<legged_estimation::ParamConfig>::CallbackType cb = [this](auto&& PH1, auto&& PH2) {
      dynamicCallback(std::forward<decltype(PH1)>(PH1), std::forward<decltype(PH2)>(PH2));
  };
  dynamic_srv_->setCallback(cb);
}

void KalmanFilterEstimate::dynamicCallback(legged_estimation::ParamConfig& config, uint32_t /*level*/){

  accXBias_ = config.accXBias;
  accYBias_ = config.accYBias;

  vXBias_ = config.vXBias;
  vYBias_ = config.vYBias;

  imuProcessNoisePosition_ = config.imuProcessNoisePosition;
  imuProcessNoiseVelocity_ = config.imuProcessNoiseVelocity; 
  footProcessNoisePosition_ = config.footProcessNoisePosition;
  footSensorNoisePosition_ = config.footSensorNoisePosition;
  footSensorNoiseVelocity_ = config.footSensorNoiseVelocity; 
  footHeightSensorNoise_ = config.footHeightSensorNoise;

  ROS_INFO_STREAM("\033[32m [KF]: Update the param. \033[0m");
}
