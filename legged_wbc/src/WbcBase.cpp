...
//    ros::NodeHandle nh("wbc");
    ros::NodeHandle nh = ros::NodeHandle(controller_nh, "wbc");;
    desiredPub_ = nh.advertise<ocs2_msgs::mpc_observation>("/legged_wbc_desired", 1);
    measurePub_ = nh.advertise<ocs2_msgs::mpc_observation>("/legged_wbc_measure", 1);

    dynamic_srv_ = std::make_shared<dynamic_reconfigure::Server<legged_wbc::WeightConfig>>(nh);
    dynamic_reconfigure::Server<legged_wbc::WeightConfig>::CallbackType cb = [this](auto&& PH1, auto&& PH2) {
        dynamicCallback(std::forward<decltype(PH1)>(PH1), std::forward<decltype(PH2)>(PH2));
    };
    dynamic_srv_->setCallback(cb);
}

void WbcBase::dynamicCallback(legged_wbc::WeightConfig &config, uint32_t) {
...
//    eulerAnglesDesired << da_ee_(0), da_ee_(1), da_ee_(2);
...
//    b = baseAngularKp_  * error + baseAngularKd_ * (- vMeasuredGlobal)
//        - base_dj_.block(3, 0, 3, info_.generalizedCoordinatesNum) * vMeasured_;
...
Task WbcBase::formulateBaseAccelTempTask() {
    matrix_t a(6, numDecisionVars_);
    a.setZero();
    a.block(0, 0, 6, 6) = matrix_t::Identity(6, 6);
    a(2, 2) = 0.;
